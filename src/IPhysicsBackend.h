...
enum class PhysicsCollisionPhase {
    Enter,
    Stay,
    Exit
};
...
struct PhysicsCollisionEvent {
    int objectAId = -1;
    int objectBId = -1;
    PhysicsCollisionPhase phase = PhysicsCollisionPhase::Enter;
};
...
public:
    ...
    // Rebuild one active actor after authoring data such as mesh collision
    // changes. Backends may return true when the actor is not currently active;
    // it will be created from the updated data on the next play start.
    virtual bool refreshObject(const SceneObject& obj) = 0;
    ...
    virtual void drainCollisionEvents(std::vector<PhysicsCollisionEvent>& outEvents) = 0;
};
