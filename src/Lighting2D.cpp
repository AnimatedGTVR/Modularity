...
namespace {
    ...
    bool BoundsIntersectViewport(const glm::vec2& boundsMin, const glm::vec2& boundsMax, int width, int height) {
        const float viewportWidth = static_cast<float>(std::max(1, width));
        const float viewportHeight = static_cast<float>(std::max(1, height));
        return boundsMax.x >= 0.0f && boundsMin.x <= viewportWidth && boundsMax.y >= 0.0f && boundsMin.y <= viewportHeight;
    }
...
void Lighting2DRenderer::ensureTarget(RenderTarget& target, int width, int height) {
    ...
    if (target.fbo != 0 && target.width == safeWidth && target.height == safeHeight) return;
...
void Lighting2DRenderer::renderLayer(const Light2DRenderRequest& request, Renderer& renderer, const LayerBatch& batch, float lightingScale, bool clearLightBuffers) {
    if (clearLightBuffers) clearLightTargets();
