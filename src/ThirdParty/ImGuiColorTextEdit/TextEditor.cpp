...
TextEditor::TextEditor()
	...
	, mAllowTabInput(true)
	, mAllowArrowNavigation(true)
	, mAllowEnterInput(true)
	, mSmartTabDelete(true)
...
std::string TextEditor::GetWordUnderCursorPublic() const
{
	return GetWordUnderCursor();
}

std::string TextEditor::GetWordAtPublic(const Coordinates& aCoords) const
{
	return GetWordAt(aCoords);
}
...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z))
			...
			Redo();
		else if (mAllowArrowNavigation && !ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_UpArrow))
			MoveUp(1, shift);
		else if (mAllowArrowNavigation && !ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_DownArrow))
			MoveDown(1, shift);
		...
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Backspace))
		{
			if (mSmartTabDelete)
			{
				...
				int tabSize = std::max(1, mTabSize);
				bool handled = false;
				if (col > 0 && (col % tabSize) == 0)
				{
					auto& line = mLines[cursor.mLine];
					int startIndex = GetCharacterIndex(Coordinates(cursor.mLine, col - tabSize));
					int endIndex = GetCharacterIndex(cursor);
					if (startIndex >= 0 && endIndex >= 0 && endIndex <= (int)line.size() && startIndex < endIndex)
					{
						bool allSpaces = true;
						...
						if (allSpaces)
						{
							Coordinates start(cursor.mLine, col - tabSize);
							Coordinates end(cursor.mLine, col);
			...
			SelectAll();
		else if (mAllowEnterInput && !IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Enter))
			EnterCharacter('\n', false);
		else if (mAllowTabInput && !IsReadOnly() && !ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_Tab))
