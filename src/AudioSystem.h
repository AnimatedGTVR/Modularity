...
public:
    ...
    // pitch also shifts playback rate, which is what you want for footstep
    // variation - the same clip at 0.95 and 1.05 reads as two different steps.
    bool playOneShot(const std::string& path, float volume = 1.0f, float pitch = 1.0f);
    void stopPreview();
    ...
    // Global AudioFX chain applied to the whole master bus. Editable at runtime.
    void setGlobalAudioFX(const AudioFXChain& chain);
    AudioFXChain getGlobalAudioFX() const { return globalFXChain; }

    // ma_node wrapper that runs an AudioFXProcessor. Used for both the global bus
    // and per-source chains. Kept public because miniaudio needs the layout.
    struct AudioFXNode {
        ma_node_base baseNode;
        AudioFXProcessor* processor = nullptr;
        int channels = 0;
    };
...
private:
    ...
    struct ActiveSound {
        ...
        // Per-source AudioFX. The node is inserted between the sound and the
        // master bus when the chain is active, and torn down when it is cleared.
        std::unique_ptr<AudioFXProcessor> fxProcessor;
        AudioFXNode fxNode{};
        bool fxNodeReady = false;
        std::string fxSignature; // serialized chain, to detect edits
    };
    struct OneShotSound {
        ma_sound sound{};
        std::shared_ptr<DecodedAudioData> decodedData;
    ...
    AudioFXProcessor globalFXProcessor;
    AudioFXNode globalFXNode{};
    bool globalFXReady = false;
    AudioFXChain globalFXChain;
    std::string globalFXSignature;      // cache to avoid rebuilding the global chain per frame
    bool globalFXFromComponent = false; // true while a global AudioFX component drives the bus
    ...
    // AudioFX helpers.
    ma_node* masterBusInputNode() const; // where sounds/fx nodes feed into
    void initGlobalFXNode(ma_uint32 channels, ma_uint32 sampleRate);
    void updateGlobalFXFromObjects(const std::vector<SceneObject>& objects);
    void updateSourceFX(const SceneObject& obj, ActiveSound& snd);
    void destroySourceFX(ActiveSound& snd);
    void teardownSound(ActiveSound& snd); // uninit sound + fx node + decoded data
};
