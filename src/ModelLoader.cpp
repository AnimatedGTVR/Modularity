...
// Blender 3.0+ writes compressed .blend files with Zstandard, which Assimp cannot read.
// This used to shell out to the zstd CLI - fine on Linux, where zstd is present on most
// systems, and silently broken on Windows, where nothing ships it. It now decodes in
// process against the vendored single-file decompressor (src/ThirdParty/zstd).
//
// Recorded so the dead ends are not re-attempted: Windows' bundled tar.exe is bsdtar and
// libarchive *can* decode zstd, but bsdtar only auto-detects archive formats and rejects
// a raw .zst stream ("Unrecognized archive format"). .NET has Deflate/GZip/Brotli, no zstd.
//
// Streaming, not ZSTD_decompress: Blender writes the file as several frames followed by a
// skippable seek-table frame so it can seek within its own saves. A one-shot call decodes
// only the first frame - measured on a real 480 KB file, 36 KB of 1.5 MB - and
// ZSTD_getFrameContentSize() likewise reports only that first frame. The streaming API
// walks concatenated frames and skips skippable ones by itself.
bool DecompressZstdBlend(const std::string& filepath, fs::path& outPath, std::string& errorMsg) {
    ...
    if (!source) {
        errorMsg = "Blender file uses Zstandard compression, but the file could not be opened for reading.";
        return false;
    }

    ZSTD_DStream* stream = ZSTD_createDStream();
    if (stream == nullptr) {
        errorMsg = "Blender file uses Zstandard compression, but the decompressor could not be created.";
        return false;
    }
    const size_t initResult = ZSTD_initDStream(stream);
    if (ZSTD_isError(initResult)) {
        errorMsg = std::string("Failed to start Zstandard decompression: ") +
                   ZSTD_getErrorName(initResult);
        ZSTD_freeDStream(stream);
        return false;
    }

    std::ofstream destination(outPath, std::ios::binary | std::ios::trunc);
    if (!destination) {
        errorMsg = "Blender file uses Zstandard compression, but a temporary file for the "
                   "decompressed data could not be created.";
        ZSTD_freeDStream(stream);
        return false;
    }
    ...
    while (!failed) {
        source.read(inBuffer.data(), static_cast<std::streamsize>(inBuffer.size()));
        const std::streamsize readCount = source.gcount();
        if (readCount <= 0) {
            break;
        }

        ZSTD_inBuffer input{inBuffer.data(), static_cast<size_t>(readCount), 0};
        while (input.pos < input.size) {
            ZSTD_outBuffer output{outBuffer.data(), outBuffer.size(), 0};
            const size_t status = ZSTD_decompressStream(stream, &output, &input);
            if (ZSTD_isError(status)) {
                errorMsg = std::string("Failed to decompress Zstandard-compressed .blend: ") +
                           ZSTD_getErrorName(status);
                failed = true;
                break;
            }
            if (output.pos > 0) {
                destination.write(outBuffer.data(), static_cast<std::streamsize>(output.pos));
                if (!destination) {
                    errorMsg = "Failed to write the decompressed .blend to a temporary file "
                               "(out of disk space?).";
                    failed = true;
                    break;
                }
    ...
    if (!failed && totalWritten == 0) {
        errorMsg = "Blender file uses Zstandard compression, but decompression produced no data.";
        failed = true;
    }
    ...
    if (!fs::exists(outPath, ec) || ec) {
        errorMsg = "Blender file uses Zstandard compression, but the temporary decompressed "
                   ".blend was not created.";
        return false;
    }
...
const aiScene* ReadSceneWithBlendFallback(Assimp::Importer& importer,
                                          ...
                                          std::string& errorMsg) {
    // Route every open Assimp makes - the model itself and each sibling it pulls in - through
    // the portable-path rescue, so a scene carrying absolute paths from another machine still
    // resolves. Both import entry points reach Assimp through here, so this is the one place
    // the handler has to be attached. See AssimpPortableIO.h for why it exists.
    Modularity::AttachPortableAssetIO(importer);

    // The .blend helpers below sniff and decompress with std::ifstream rather than through
    // Assimp, so they need the path resolved directly as well as via the handler.
    const std::string filepath = Modularity::Platform::ResolveAssetPath(rawFilepath);
    ...
    // Assimp has already been given a go at this file above, so reaching here means it
    // genuinely could not read it. Report the format note plus what Assimp said.
    if (DescribeUnsupportedBlendFormat(fs::path(filepath), errorMsg)) {
        ...
        if (!assimpError.empty()) {
            errorMsg += " (tried anyway - Assimp reported: " + assimpError + ")";
        }
        return nullptr;
    }
    ...
    if (compression == BlendCompressionKind::ZStd) {
        ...
        // Try the import even when the header says Blender 5+, rather than refusing on
        // sight. It is expected to fail - that release changed the container, not just
        // the header: a 17-byte file header and a wider block header whose address
        // field looks like a sequential id rather than a memory address, which is the
        // thing Assimp resolves every pointer against. But attempting costs one read
        // and means a file that happens to be readable is not turned away untried.
        scene = importer.ReadFile(tempFile.path.string(), importFlags);
        if (!IsUsableScene(scene)) {
            std::string formatNote;
            if (DescribeUnsupportedBlendFormat(tempFile.path, formatNote)) {
                // Lead with the actionable note; keep Assimp's text for diagnosis.
                errorMsg = formatNote + " (tried anyway - Assimp reported: " +
