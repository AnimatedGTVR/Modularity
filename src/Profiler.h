...
public:
    ...
    // wholeFrame: this query spans every GL command the frame issues on the main
    // context, so the result is the frame's true GPU cost rather than one pass.
    // Nested begins are ignored while a query is active, so a whole-frame query
    // automatically absorbs the per-pass ones instead of fighting them.
    bool beginOpenGlGpuFrame(bool wholeFrame = false);
