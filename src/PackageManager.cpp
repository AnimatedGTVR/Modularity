...
#include <vector>
#if defined(_WIN32)
// For the windowless CreateProcess used by runCommand.
...
void PackageManager::setProjectRoot(const fs::path& root) {
    ...
    buildRegistry();
#endif
    const auto __t1 = std::chrono::steady_clock::now();
    projectRoot = root;
    manifestPath = projectRoot / "packages.modu";
#if !MODULARITY_RUNTIME_ONLY
    loadManifest();
#endif
    const auto __t2 = std::chrono::steady_clock::now();
    const double buildMs = std::chrono::duration<double, std::milli>(__t1 - __t0).count();
    const double manifestMs = std::chrono::duration<double, std::milli>(__t2 - __t1).count();
    std::fprintf(stderr,
                 "[PackageManager] setProjectRoot: buildRegistry=%.1f ms loadManifest=%.1f ms\n",
                 buildMs, manifestMs);
    std::fflush(stderr);
}
...
void PackageManager::refreshRegistry() {
    ...
    return;
#else
    // Explicit user action (Package Manager / Registry windows), so this is the
    // one path allowed to go to the network.
    allowRemoteRegistryFetch = true;
    buildRegistry();
    allowRemoteRegistryFetch = false;
...
bool PackageManager::loadRegistryMetadata(const fs::path& engineRoot) {
    ...
    // Cache first when a fetch is not explicitly wanted. The remote listing is a
    // curl with a 10s connect timeout and two retries, and it used to run inside
    // setProjectRoot -- so every project open blocked the main thread on an HTTP
    // request, which is what froze the editor for several seconds before the
    // window could even draw a frame.
    std::error_code cacheEc;
    const fs::path cachedListing = packageCacheFolder() / "Metadata" / "PackageManagerInfo.modu";
    if (!allowRemoteRegistryFetch && fs::exists(cachedListing, cacheEc) && !cacheEc) {
        listingPath = cachedListing;
        usingRemoteListing = true;
    } else if (allowRemoteRegistryFetch && downloadRemoteRegistryListing(listingPath, remoteLog)) {
        usingRemoteListing = true;
    ...
    } else if (downloadRemoteRegistryListing(listingPath, remoteLog)) {
        // Bootstrap: nothing cached and no local checkout, so this is a first run
        // and the fetch is the only way to get a listing at all. Pays the network
        // cost once, after which the cache branch above keeps project opens free.
        usingRemoteListing = true;
    } else {
    ...
    // ModuEngineVersion(...) token of the block currently being parsed, plus the
    // block each already-registered package id came from. Used to resolve
    // duplicate ids by version rather than by file order (see finalizePackage).
    std::string currentBlockVersion;
    std::unordered_map<std::string, std::string> entryBlockVersion;
    ...
    auto finalizePackage = [&]() {
        ...
        if (existing != registry.end()) {
            // Only let a newer version block win. This used to be positional
            // (last block parsed overwrote the entry), which silently depended on
            // blocks being written in ascending order in PackageManagerInfo.modu.
            // isCompatibleVersionString(rule, current) is "current >= rule" and
            // compares numerically component-wise, so 7.0 beats 6.7 and a future
            // 7.10 beats 7.9 - a lexicographic compare would get both wrong.
            const auto prior = entryBlockVersion.find(currentPackage.id);
            const bool newBlockIsNewer =
                prior == entryBlockVersion.end() ||
                prior->second.empty() ||
                currentBlockVersion.empty() ||
                isCompatibleVersionString(prior->second, currentBlockVersion);
            if (!newBlockIsNewer) {
                currentPackage = PackageInfo{};
                return;
            }
            entryBlockVersion[currentPackage.id] = currentBlockVersion;
    ...
    while (std::getline(file, rawLine)) {
        ...
        if (!inVersionBlock && lineContainsBlockHeader(line, "ModuEngineVersion(")) {
            ...
            if (pendingVersionBlock) {
                currentBlockVersion = versionToken;
            }
...
std::string PackageManager::currentEngineVersion() {
    // Engine version of record for package compatibility. A registry version
    // block is included when this is >= the block's version, so bumping to 7.0
    // is what makes the ModuEngineVersion("7.0") block visible at all; 6.4/6.5/
    // 6.7 blocks keep matching, so older releases stay resolvable.
    //
    // Reads (findVersionedPackageAtAnyVersion, resolveRegistrySourcePath) scan
    // every version directory, so raising this does not orphan packages already
    // installed under an older engine-version folder. Only the *write* location
    // for new global installs moves to 7.0.0/.
    return "7.0.0";
}
...
#ifdef _WIN32
    // CreateProcess with CREATE_NO_WINDOW rather than _popen: the editor is a
    // GUI-subsystem process with no console, so _popen allocates one and a black
    // console window flashes on screen for the life of the command (curl fetches,
    // archive extraction). Same capture semantics, no window.
    {
        ...
        // cmd /d /s /c so redirection and quoting behave as the callers expect.
        std::string commandLine = "cmd.exe /d /s /c \"" + command + "\"";
        std::vector<char> mutableCommand(commandLine.begin(), commandLine.end());
        mutableCommand.push_back('\0');

        PROCESS_INFORMATION pi{};
        const BOOL launched = CreateProcessA(nullptr, mutableCommand.data(), nullptr, nullptr,
