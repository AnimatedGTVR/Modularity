...
namespace Modularity::CrashReporter {
    ...
    int RunProtected(const std::function<int()>& entryPoint) {
        ...
        catch (const std::exception& e) {handleCrash("Unhandled exception", e.what(), EXIT_FAILURE);}
        catch (...) {handleCrash("Unhandled exception", "Non-standard exception", EXIT_FAILURE);}
    }
