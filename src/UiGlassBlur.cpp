...
void Install()
{
    if (installed || failed)
        return;
    // MODULARITY_DISABLE_GLASS_BLUR=1: kill switch for perf work. The capture runs
    // once per translucent top-level window, so this is the cheapest way to see how
    // much of a frame's GPU time the editor's glass panels actually cost.
    if (const char* off = std::getenv("MODULARITY_DISABLE_GLASS_BLUR")) {
        if (*off && *off != '0') {
            failed = true;
            return;
        }
...
{
    if (drawList == nullptr || !installed || failed || texA == 0)
        return false;
    // same order ModuGUI uses for window glass: capture, hand the GL state back to the
    // backend, then the caller's image samples what we just blurred.
    drawList->AddCallback(&CaptureCallback, nullptr);
...
ImTextureID BackdropTexture()
{
    if (!installed || failed)
        return 0;
    return (ImTextureID)(intptr_t)texA;
}
