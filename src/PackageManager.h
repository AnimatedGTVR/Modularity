...
public:
    ...
    // Engine version of record for package compatibility, also what the
    // launcher shows in its status bar. Public so UI code has one source of
    // truth instead of a second hardcoded version string.
    static std::string currentEngineVersion();
    // Where global (all-project) registry installs live on this machine.
    fs::path globalPackagesRoot() const { return globalPackagesFolder(); }

private:
    // Opening a project must not block on the network. When false, the registry
    // listing is taken from the on-disk cache (or a local checkout) and no HTTP
    // request is made; refreshRegistry() sets it true so the Package Manager
    // windows still fetch a fresh listing on demand.
    bool allowRemoteRegistryFetch = false;
    void buildRegistry();
    void loadManifest();
