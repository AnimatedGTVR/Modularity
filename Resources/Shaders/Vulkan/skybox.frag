...
vec3 getSkyColor(vec3 dir, float tod) {
    ...
    float sunVisibility = smoothstep(-0.12, 0.15, sunDir.y);
    ...
    if (night > 0.0) {
        ...
        stars = pow(stars, 1.4) * night;
        stars *= smoothstep(-0.1, 0.25, height);

        vec3 galaxyAxis = normalize(vec3(0.2, 0.85, -0.5));
        float galaxyBand = 1.0 - abs(dot(dir, galaxyAxis));
        galaxyBand = pow(max(galaxyBand, 0.0), 2.5);
...
void main() {
    ...
    fragColor = vec4(getSkyColor(dir, timeOfDay), 1.0);
}
