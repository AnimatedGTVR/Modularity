...
public class SampleInspector
{
    ...
    [DragSpeed(0.1f)] private Vec3 TestValue = new Vec3(0f, 1f, 0f);
    private ModuObject targetObject;

    void Begin(IntPtr ctx, float deltaTime)
    {
    ...
    void Spec(IntPtr ctx, float deltaTime)
    {
        ApplyAutoRotate(new Context(ctx), deltaTime);
    }

    void TestEditor(IntPtr ctx, float deltaTime)
    {
        ApplyAutoRotate(new Context(ctx), deltaTime);
    }

    void TickUpdate(IntPtr ctx, float deltaTime)
    {
        ApplyAutoRotate(new Context(ctx), deltaTime);
    }
