...
#
# Usage: tools/moduobj-selftest/run.sh
set -euo pipefail
cd "$(dirname "$0")"
REPO_ROOT="$(cd ../.. && pwd)"
BUILD_DIR="${TMPDIR:-/tmp}/modularity-moduobj-selftest"
...
(cd "$REPO_ROOT/build" && g++ -std=c++17 -O0 -g \
    ...
    "$REPO_ROOT/tools/moduobj-selftest/moduobj_test.cpp" \
    -o "$BUILD_DIR/moduobj_test" \
    -Wl,--start-group $LINK_LIBS -Wl,--end-group)

"$BUILD_DIR/moduobj_test" "$BUILD_DIR/work"
